...
from app.api.permissions import DeskPermissions, TaskPermissions, TaskTypePermissions
...
from app.models.task_type_models import (
    ...
    check_belong_task_type_to_user,
)
...
def task_type_create_endpoint(
    ...
    """
    db_task_type: TaskType = create_task_type_model(data=task_type, db=db)
...
def get_task_type_list(
        desk_id: int, is_show: typing.Optional[bool] = None,
        current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> typing.List[TaskTypeSchema]:
    task_type_models_list = get_task_types_by_desk_id(desk_id=desk_id, is_show=is_show, db=db)
...
def edit_task_type(
        task_type_id: int, data: EditTaskTypeSchema,
        current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> dict:
    edit_task_type_model(task_type_id, data, db)
    return {}
...
def delete_task_type(
    ...
    delete_task_type_model(task_type_id, db)
    return {}
